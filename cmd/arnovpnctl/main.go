// Command arnovpnctl is a small helper for operators: it prints the connect
...
// server, so "arnovpnctl uri" always matches what the running server accepts.
package main
...
func usage() {
	fmt.Fprintf(os.Stderr, `arnovpnctl — ArnoVPN operator helper

Usage:
  arnovpnctl uri            print the connect URI
  arnovpnctl qr             print a scannable QR code
  arnovpnctl png <file>     write the connect QR to a PNG file
  arnovpnctl genpsk         print a fresh base64 pre-shared key
  arnovpnctl show           print the resolved configuration

Configuration is read from the same environment variables as the server
(ARNO_DOMAIN, ARNO_WS_PATH, ARNO_PSK / state file, ...).
`)
}
