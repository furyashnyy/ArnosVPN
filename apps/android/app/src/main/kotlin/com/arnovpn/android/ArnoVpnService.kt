...
import com.arnovpn.android.protocol.Crypto
import com.arnovpn.android.protocol.Profile
import com.arnovpn.android.protocol.Session
...
/**
 * ArnoVpnService establishes the local TUN interface and bridges it to the
 * ArnoVPN server over a WSS tunnel. All routes point into the tunnel, so every
 * app's traffic exits from the server and the device's apparent public IP
 * becomes the server's.
 */
class ArnoVpnService : VpnService() {

    companion object {
        const val ACTION_CONNECT = "com.arnovpn.android.CONNECT"
        const val ACTION_DISCONNECT = "com.arnovpn.android.DISCONNECT"
        const val ACTION_STATE = "com.arnovpn.android.STATE"
        ...
        private const val TAG = "ArnoVpn"
        private const val CHANNEL_ID = "arnovpn"
    ...
    private fun startUplink(fd: ParcelFileDescriptor) {
        ...
        }.apply { name = "arno-uplink"; isDaemon = true; start() }
    }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (nm.getNotificationChannel(CHANNEL_ID) == null) {
                nm.createNotificationChannel(
                    NotificationChannel(CHANNEL_ID, "ArnoVPN", NotificationManager.IMPORTANCE_LOW),
                )
            }
        }
    }
